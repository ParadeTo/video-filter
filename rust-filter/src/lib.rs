...
#[wasm_bindgen]
pub fn filter(ctxHidden: &CanvasRenderingContext2d, ctx: &CanvasRenderingContext2d, width: u32, height: u32, kernel: Vec<f32>) {
    set_panic_hook();
    let image_data = ctxHidden
    ...
    ctx.put_image_data(&image_data, 0.0, 0.0).expect("put image data panic")
}
